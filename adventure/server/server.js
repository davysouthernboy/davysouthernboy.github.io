...
//  Run: npm install express socket.io && node server.js
// ============================================================
...
function buildMap() {
  ...
  for (let y = 0; y < MAP_H; y++) {
    ...
    for (let x = 0; x < MAP_W; x++) {
      // border walls
      if (x === 0 || y === 0 || x === MAP_W-1 || y === MAP_H-1) { m[y].push(1); continue; }
      // internal obstacles
      if ((x === 6 && y >= 4 && y <= 10) || (x === 12 && y >= 12 && y <= 19) ||
          (x === 20 && y >= 3 && y <= 9) || (x === 25 && y >= 14 && y <= 20) ||
          (y === 8  && x >= 14 && x <= 18) || (y === 16 && x >= 6 && x <= 11)) { m[y].push(1); continue; }
      // water patches
      if ((x >= 15 && x <= 17 && y >= 18 && y <= 21) ||
          (x >= 2  && x <= 4  && y >= 14 && y <= 17)) { m[y].push(2); continue; }
      // sand
      if ((x >= 22 && x <= 28 && y >= 17 && y <= 22)) { m[y].push(3); continue; }
      m[y].push(0);
    }
  }
  return m;
...
class GameRoom {
  ...
  spawnEnemies() {
    return ENEMY_STARTS.map((s, i) => ({
      ...
      dir: 'down', moveTimer: 0, moveDir: { x: 0, y: 1 },
      alive: true, type: i % 3  // 0=octorok 1=moblin 2=like-like
    }));
  }
  ...
  addPlayer(socketId, name, colorIndex) {
    ...
    const start = idx === 0 ? { x: 3*TILE, y: 3*TILE } : { x: 29*TILE, y: 21*TILE };
    this.players[socketId] = {
      id: socketId, name, colorIndex,
      x: start.x, y: start.y,
      hp: 6, maxHp: 6,
      dir: 'down',
  ...
  update() {
    ...
    for (const p of plist) {
      ...
      if (p.swordTimer > 0) { p.swordTimer -= TICK_MS; if (p.swordTimer <= 0) p.sword = false; }
    }

    // ── move arrows ───────────────────────────────────────
    this.arrows = this.arrows.filter(a => {
      a.x += a.vx; a.y += a.vy; a.life -= TICK_MS;
      if (a.life <= 0 || this.wallAt(a.x, a.y, 4)) return false;
      for (const e of this.enemies) {
        if (!e.alive) continue;
        if (this.dist(a.x, a.y, e.x+16, e.y+16) < 20) {
          e.hp--; if (e.hp <= 0) { e.alive = false; this.awardKill(a.ownerId); }
          return false;
        }
      }
      return true;
    ...
    // hit cooldown
    for (const p of plist) {
      if (p.hitCooldown > 0) p.hitCooldown -= TICK_MS;
    }
    ...
    for (const p of plist) {
      ...
      for (const e of this.enemies) {
        ...
        if (this.dist(sx, sy, e.x+16, e.y+16) < 28) {
          e.hp--;
          if (e.hp <= 0) { e.alive = false; this.awardKill(p.id); }
        }
      }
    }
    ...
    io.to(this.id).emit('state', {
      players: plist.map(p => ({
        ...
        sword: p.sword, dead: p.dead,
        bombs: p.bombs, arrows: p.arrows, score: p.score,
  ...
  awardKill(playerId) {
    if (this.players[playerId]) this.players[playerId].score += 10;
  }
  ...
  handleInput(socketId, input) {
    ...
    if (input.up)    { p.vy = -spd; p.dir = 'up'; }
    if (input.down)  { p.vy =  spd; p.dir = 'down'; }
    if (input.left)  { p.vx = -spd; p.dir = 'left'; }
    if (input.right) { p.vx =  spd; p.dir = 'right'; }

    if (input.sword && !p.sword) {
      p.sword = true; p.swordTimer = SWORD_MS;
    }
    if (input.shootArrow && p.arrows > 0) {
      ...
      this.arrows.push({ id: `ar${Date.now()}`, x: p.x+8, y: p.y+8, vx: dvx, vy: dvy, life: 2000, ownerId: socketId });
    }
    if (input.bomb && p.bombs > 0) {
      p.bombs--;
      // simple bomb: hurt all nearby enemies after 1.5s
      const bx = p.x, by = p.y;
      setTimeout(() => {
        for (const e of this.enemies) {
          if (!e.alive) continue;
          if (this.dist(bx, by, e.x, e.y) < 64) {
            e.hp -= 2;
            if (e.hp <= 0) { e.alive = false; this.awardKill(socketId); }
          }
        }
...
function getOrCreateRoom(socket, playerName, mode) {
  if (mode === 'solo') {
...
io.on('connection', (socket) => {
  ...
  socket.on('join', ({ name, mode }) => {
    ...
    myRoom = getOrCreateRoom(socket, name || 'Hero', mode || 'multi');
    myRoom.addPlayer(socket.id, name || 'Hero', colorIndex);
    ...
    socket.emit('joined', {
      ...
      colorIndex,
      playerCount,
      waiting: mode !== 'solo' && playerCount < 2
    });

    if (mode === 'solo' || playerCount === 2) {
      myRoom.start();
  ...
  socket.on('input', (input) => {
    if (myRoom) myRoom.handleInput(socket.id, input);
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (waitingRoom && waitingRoom.players[socket.id]) {
      waitingRoom = null;
    }
  });
});
...
server.listen(PORT, () => console.log(`🗡️  Zelda server running on http://localhost:${PORT}`));
